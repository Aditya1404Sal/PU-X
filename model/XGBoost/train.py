...
# Define 'strong' stocks as those in the top 25% of strength scores
strength_threshold = df['strength_score'].quantile(0.75)
...
label_strength = {
    'IN-L': 1,  # Large cap = strong
    'IN-M': 1,  # Mid cap = strong
    'IN-S': 0,  # Small cap = not strong
    'IN-Mi': 0  # Micro cap = not strong
}

# For stocks with labels, use them to set strength
labeled_mask = df['intrinsic_label'].isin(['IN-L', 'IN-M', 'IN-S', 'IN-Mi'])
df.loc[labeled_mask, 'labeled_strength'] = df.loc[labeled_mask, 'intrinsic_label'].map(label_strength)
...
# Check correlation between our strength score and labeled strength
labeled_correlation = df.loc[labeled_mask, ['strength_score', 'labeled_strength']].corr().iloc[0, 1]
print(f"Correlation between our strength score and labeled strength: {labeled_correlation:.4f}")
...
top_n_features = 20  # Adjust based on feature importance later
...
top_stocks = df.sort_values('strength_probability', ascending=False).head(50)

print("\n===== TOP 20 INTRINSICALLY STRONG STOCKS =====")
...
top_stocks[['symbol', 'company_name', 'company_industry', 'sector', 'strength_probability', 
           ...
           'profit_margin', 'year_change_percent']].to_csv('top_strong_stocks.csv', index=False)
print("\nSaved top strong stocks to 'top_strong_stocks.csv'")
...
with open('stock_strength_model_report.txt', 'w') as f:
    ...
    f.write("TOP INTRINSICALLY STRONG STOCKS\n")
    f.write("----------------------------\n")
    f.write("Top 50 stocks by predicted strength probability:\n\n")
...
print("3. A list of the top 50 predicted strong stocks")
